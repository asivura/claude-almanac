...
**Bindings source of truth**: `site/wrangler.toml` controls all bindings. The Cloudflare dashboard UI is locked for this project — any binding must be added to `wrangler.toml` and deployed via CI.

### Analytics Engine binding

All per-request tracking and the `/analytics` dashboard read from a single Cloudflare Analytics Engine dataset.
...
Declared in `site/wrangler.toml` under `[[analytics_engine_datasets]]`. The binding is optional at runtime — when absent (local dev), tracking calls are a silent no-op.

### Pages Secrets for the analytics dashboard

The `/analytics` dashboard queries the Analytics Engine SQL API server-side. It needs two Pages Secrets:
...
- **Cloudflare Web Analytics** — Analytics Engine covers our needs
- **D1 Database** — event logging was migrated to Analytics Engine; no D1 binding is needed
- **KV Namespace** — aggregate caching was removed along with D1; no KV binding is needed
